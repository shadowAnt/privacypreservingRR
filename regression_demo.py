...
class Processing():
    ...
    def feature_engineer_1(self, path):
        ...
        train_test.loc[train_test['缴费用户最近一次缴费金额（元）'] == 0, '缴费用户最近一次缴费金额（元）'] = train_test['缴费用户最近一次缴费金额（元）'].mode()
        train_test['用户上次缴费前欠费多少'] = train_test['缴费用户最近一次缴费金额（元）'] - train_test['用户账单当月总费用（元）'] - train_test[
            '用户当月账户余额（元）']
        ...
        train_test['各类应用使用总次数'] = train_test['当月出行类应用使用次数'] + train_test['当月金融理财类应用使用总次数'] + train_test['当月网购类应用使用次数'] + \
                                  train_test['当月视频资讯类应用使用次数'] + train_test['当月物流快递类应用使用次数']
...
if __name__ == "__main__":
    ...
    x_train, x_test, y_train, y_test = train_test_split(train_test[features], train_test['信用分'], test_size=0.2,
                                                        random_state=2019)
    # 线形回归
    ...
    x_list = ['linear_predict',  'ridge_predict']
